...
// function computeBoxPlacements(groupedByStop, vehicleDims) {
...
//   for (let z = 0; z < truckHeight; z += 0.01) {
//     grid.push([]);
//   }

//   // Sort stops in reverse (FILO)
//   const sortedStops = Object.keys(groupedByStop)
//     .map(Number)
//     .sort((a, b) => b - a);
...
//     const packagesAtStop = groupedByStop[stop];

//     for (const pkg of packagesAtStop) {
//       const lines = pkg?.lines || [];

//       for (const item of lines) {
//         const { quantity, packagingDimensions, prod_ID } = item;
//         const { lengthM, widthM, heightM } = packagingDimensions;

//         for (let i = 0; i < quantity; i++) {
//           let placed = false;

//           // Try to fit this box layer by layer
//           for (let z = 0; z <= truckHeight - heightM; z += 0.01) {
//             for (let y = 0; y <= truckWidth - widthM; y += 0.01) {
//               for (let x = 0; x <= truckLength - lengthM; x += 0.01) {
//                 if (canPlace(placements, x, y, z, lengthM, widthM, heightM)) {
//                   placements.push({
...
//           if (!placed) {
//             console.warn(`Box for ${prod_ID} at stop ${stop} could not be placed.`);
//           }
//         }
//       }
//     }
...
// function canPlace(existingBoxes, x, y, z, length, width, height) {
//   for (const box of existingBoxes) {
//     const [bx, by, bz] = box.position;
//     const [bl, bw, bh] = box.dimensions;
...
//           return {
//             prod_ID: line.prod_ID, // ✅ Include prod_ID here
//             quantity: line.quantity,
//             packagingDimensions: {
//               lengthM: parseDimension(`${info.pack_length} ${info.dimensions_uom}`),
//               widthM: parseDimension(`${info.pack_width}  ${info.dimensions_uom}`),
//               heightM: parseDimension(`${info.pack_height} ${info.dimensions_uom}`)
//             }
//           };
//         }).filter(Boolean);
//         pkgInfoMapForAlloc[pkgID] = { lines };
...
//       //       boxes.push({
//       //         dimensions: [packagingDimensions.widthM, packagingDimensions.lengthM, packagingDimensions.heightM],
//       //         prod_ID: line.prod_ID
//       //       });
//       //     }
//       //   });
...
//             quantity: line.quantity,
//             packagingDimensions,
//             stacking_factor: +productMap[line.prod_ID]?.stacking_factor || 1,
//             stop_number: a.loadArrangement?.find(l => l.packages.includes(pkgID))?.stop || 1
//           });
//         });
...
//       for (const [pkgID, pkgData] of Object.entries(pkgInfoMapForAlloc)) {
//         const stopNum = a.loadArrangement?.find(l => l.packages.includes(pkgID))?.stop || 1;
//         if (!groupedByStop[stopNum]) groupedByStop[stopNum] = [];
//         groupedByStop[stopNum].push(pkgData);
//       }

//   const computedPlacements = computeBoxPlacements(groupedByStop, vehicleDims);

//       Object.assign(boxPlacements, computedPlacements);


//       const actualOccupiedVolume = a.packages.reduce((sum, pkgID) => {
//         const boxes = (boxPlacements[pkgID]?.boxes) || [];
//         return sum + boxes.reduce((sub, b) => {
//           const [l, w, h] = b.dimensions;
//           return sub + (l * w * h);
//         }, 0);
//       }, 0);

//       a.occupiedVolume = actualOccupiedVolume;
//       const occupiedPercent = usable > 0 ? +(actualOccupiedVolume / usable * 100).toFixed(2) : 0;

//       return {
//         ...a,
//         occupiedVolume: actualOccupiedVolume,
//         boxPlacements,
