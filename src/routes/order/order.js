...
router.post('/create-order', jwtAuth.verifyToken, async (req, res) => {
  ...
  try {
    const {
      packages: packageIDs,
      filters,
      // Existing draft order_ID; used only while recalculating an editable draft.
      draft_order_ID = null
    } = req.body;
    if (!packageIDs?.length) return res.status(400).json({ error: 'No packages provided.' });
    ...
    // 1) fetch packages & validate same origin/date
    const packagesData = await getPackagesByIds(packageIDs, draft_order_ID);
    const origin = packagesData[0].ship_from;
    const pickupDate = packagesData[0].pickup_date_time.split('T')[0];
    packagesData.forEach(p => {
      if (p.ship_from !== origin) throw new Error('All packages must share ship_from');
    ...
    // Business validation: packages reserved in another draft or already confirmed
    if (err?.code === 'PACKAGE_NOT_AVAILABLE') {
      logger.warn('Create-order blocked: packages unavailable', {
        blockedPackages: err.blockedPackages,
        message: err.message
      });
    ...
    if (err?.code === 'PACKAGE_NOT_FOUND') {
      return res.status(404).json({
        error: err.message,
        missingPackages: err.missingPackages
      });
    }
...
async function getPackagesByIds(packageIDs, draftOrderID = null) {
  ...
  const ph = uniquePackageIDs.map(() => '?').join(',');
  const [rows] = await db.query(
    `SELECT *
       FROM packages
      WHERE pack_ID IN (${ph})`,
  ...
  if (rows.length !== uniquePackageIDs.length) {
    const foundIDs = new Set(rows.map(row => row.pack_ID));
    const missingPackages = uniquePackageIDs.filter(id => !foundIDs.has(id));

    const err = new Error('Some requested packages were not found');
    err.code = 'PACKAGE_NOT_FOUND';
    err.missingPackages = missingPackages;
    throw err;
  }

  /*
    Packages with package_status = 'draft' are reserved.
    During edit/recalculation, only packages already reserved by the same
    draft order_ID can be reused.
  */
  let sameDraftPackages = new Set();

  if (draftOrderID) {
    const [draftRows] = await db.query(
      `SELECT allocated_packages
         FROM orders
        WHERE order_ID = ?
          AND draft = 1
        LIMIT 1`,
    ...
    if (!draftRows.length) {
      const err = new Error('Editable draft order not found');
      err.code = 'DRAFT_NOT_FOUND';
      throw err;
    }

    const oldAllocatedPackages = safeJsonParse(
      draftRows[0].allocated_packages,
      []
    );

    sameDraftPackages = new Set(
      Array.isArray(oldAllocatedPackages) ? oldAllocatedPackages : []
    );
  }

  const blockedPackages = rows
    .filter(row => {
      const status = String(row.package_status || '').toLowerCase();

      if (status === 'ordered') return true;

      if (status === 'draft') {
        return !draftOrderID || !sameDraftPackages.has(row.pack_ID);
      }
  ...
  if (blockedPackages.length) {
    const err = new Error(
      'Some packages are already reserved in a draft or confirmed order'
    );
    err.code = 'PACKAGE_NOT_AVAILABLE';
  ...
  return rows.map(row => ({
    ...
    ship_to: row.ship_to,
    destination_radius: row.destination_radius,
    products: safeJsonParse(row.product_ID),
